  ...
  try {
    ...
    if (publish === "true" && course.lectures.length === 0) {
      return res.status(400).json({
        success: false,
        message:
          "To published course , you must have at add at least one lecture",
      });
    }
...
export const removeCourse = async (req, res) => {
  try {
    ...
    const courseLectures = await lectureModel.find({
...
export const getPublishedCourse = async (_, res) => {
  try {
    const publishedCourse = await courseModel
...
export const courseRatingAndReview = async (req, res) => {
  try {
    ...
    const existingReviewAndRating = course.courseRating.find(
      (data) => data.user.toString() === userId,
    );
    ...
    const totalRatingValue = course.courseRating.reduce(
      (sum, data) => sum + Number(data.rating),
      0,
    );
    const totalRatingCount = course.courseRating.length;

    const averageRating = (totalRatingValue / totalRatingCount).toFixed(1);

    course.averageRating = averageRating;
    course.totalRating = totalRatingCount;
...
export const getUserCourseRatingById = async (req, res) => {
  try {
    ...
    const userCourseRating = course.courseRating.find(
      (data) => data.user.toString() === userId,
    );
...
export const getCourseAllReviewsAndRatings = async (req, res) => {
  try {
    ...
    const course = await courseModel
      .findById(courseId)
      .select("courseRating")
      .populate("courseRating.user", "username photo");

    if (!course) {
      return res.status(400).json({
        success: true,
        message:
          "No course found and hence failed to find course review and rating",
      });
    }
    return res.status(200).json({
      success: true,
...
export const getTopCourses = async (req, res) => {
  try {
    ...
    const courses = await courseModel
      ...
      .limit(8)
      .select("courseTitle courseThumbnail coursePrice courseLevel averageRating creator enrolledStudents")
      .populate("creator", "username photo")
      .lean();


    await redis.set("topCourses", JSON.stringify(courses), "EX", 60 * 60);    
