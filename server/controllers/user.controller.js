...
export const userProfile = async (req, res) => {
  try {
    ...
    await userModel.findOne({ _id: userId }, { enrolledCourses: 1 });
...
export const updateProfile = async (req, res) => {
  try {
    ...
    const updatedUserPfp = await userModel
      .findByIdAndUpdate(userId, UpdatePfp, { new: true })
      .select("-password"); //{new: true} taake saari chize updated dikhe
    return res.status(200).json({
      success: true,
      user: updatedUserPfp,
