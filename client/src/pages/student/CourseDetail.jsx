...
import {
  useGetCourseAllReviewsAndRatingsQuery,
  useGetCourseByIdQuery,
} from "@/features/api/courseApi";
import { useParams } from "react-router-dom";
import {
  useCreateOrderMutation,
  useGetPurchasedCourseStatusQuery,
...
const CourseDetail = () => {
  ...
  const { data: purchasedCourseData, isLoading: purcahsedIsLoading } =
    useGetPurchasedCourseStatusQuery(courseId);
  const { data: courseRatingData } =
    useGetCourseAllReviewsAndRatingsQuery(courseId);
  ...
  const sumOfAllRating = courseRatingData?.Ratings?.courseRating.reduce(
    (acc, currentVal) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      {/* Hero Section */}
      <div
        className={`bg-gradient-to-r from-indigo-900 to-purple-900 dark:from-gray-900 dark:to-gray-800 text-white transform transition-all duration-1000 ${
              ...
              <div className="flex items-center gap-6 mb-8">
                ...
                <div className="flex items-center gap-1 group">
                  <Rating
                    ...
                    readOnly
                  />
                  <span className="ml-2 group-hover:text-yellow-300 transition-colors">
                    ( {averageRating || 0} )
                  </span>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                {purchasedCourse && (
                  <button
                    ...
                    className="bg-gradient-to-r from-indigo-500 to-purple-500 dark:from-indigo-600 dark:to-purple-600 hover:from-indigo-600 hover:to-purple-600 px-8 py-4 rounded-lg font-semibold flex items-center gap-2 transform hover:scale-105 transition-all shadow-lg hover:shadow-xl"
                  >
                ...
                {purchasedCourse && (
                  <div className="relative">
                    ...
                    <div
                      className={`absolute z-10 mt-2 transition-transform transform duration-500  ${
                        showCard
            ...
            <div className="transform transition-all duration-1000 hover:cursor-pointer">
              <div className="relative bg-white dark:bg-gray-900 text-black dark:text-white backdrop-blur- rounded-xl shadow-2xl overflow-hidden border border-white/20 dark:border-gray-700 w-auto sm:w-[550px]">
                <div className="relative aspect-video cursor-pointer group">
                  <video
                    className="absolute  sm:h-full w-full "
                    controls={true}
                    ...
                    <div className="w-full">
                      {purchasedCourse ? (
                        ...
                        <button className="w-full">
                          <PurchaseCourse
                            handlePayment={handleOrderAndPayment}
                            isLoading={isLoading}
                          />
                        </button>
                      )}
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex gap-8 border-b border-gray-200 dark:border-gray-700 mb-8">
          {["overview", "reviews"].map((tab) => (
            <button
              key={tab}
              className={`pb-4 px-4 font-medium capitalize relative group ${
                activeTab === tab
            ...
            >
              {tab}
              <span
                className={`absolute bottom-0 left-0 w-full h-0.5 bg-indigo-600 dark:bg-indigo-400 transform transition-transform duration-300 ${
                  activeTab === tab
                    ? "scale-x-100"
                  ...
                  {course.lectures.map((lecture, index) => (
                    ...
                    >
                      {free ? (
                        ...
                        <LockIcon
                          size={24}
                          className="text-black dark:text-gray-400 mt-1"
                        />
                      )}
              ...
              <div className="bg-white dark:bg-gray-900 p-8 rounded-xl shadow-lg hover:shadow-xl dark:hover:shadow-2xl transition-all transform hover:scale-105">
                ...
                <div className="space-y-6">
                  {[
                    ...
                    {
                      label: "Last update",
                      value: `${course.updatedAt.split("T")[0]}`,
                    },
                  ].map((feature, index) => (
                    <div
                      key={index}
