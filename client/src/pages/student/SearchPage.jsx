        ...
        <div className='p-10 gap-10 bg-blue-50 dark:bg-gray-950'>
            <div className='min-h-screen max-w-7xl mx-auto'>
                <div  className='flex justify-center flex-wrap gap-5 '>
                   {categories.map((category) => <button onClick={()=>handleCategorySearch(category.id)} id={category.id}  key={category.id} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-300 bg-white   ${selectedCategories === category.id ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/30'
                    : 'bg-white/95 dark:bg-gray-100 backdrop-blur-sm text-gray-700 hover:bg-white' }`}>{category.label}</button>)}
                </div>

                <div className='my-10'>
                    {isLoading? [1,2,3,4].map((_,index) =><SearchCourseSkeleton key={index}/> ) : isEmpty? (<CourseNotFound/>) : (data?.courses.map((course) => <SearchedCourse course={course} key={course._id} />))}
  ...
  return (
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-gray-300 py-4 gap-4 animate-pulse">
      ...
      <div className="flex flex-col gap-3 w-full md:w-auto">
        ...
        <Skeleton className="h-4 w-1/4" />   {/* Instructor */}
