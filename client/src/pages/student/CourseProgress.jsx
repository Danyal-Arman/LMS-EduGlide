...
const CourseProgress = () => {
    ...
    const [courseComplete, { data: courseCompleteData, isSuccess, error: courseCompleteError }] = useCourseCompleteMutation();
    const [courseInComplete, { data: courseInCompleteData, isSuccess: courseInCompleteIssuccess, error: courseInCompleteError }] = useCourseInCompleteMutation()
    const [currentLecture, setCurrentLecture] = useState(null)

    useEffect(() => {
        if (isSuccess) {
            toast.success(courseCompleteData.message || "congratulation! you completed this course successfully")
            refetch()
        }
        if (courseCompleteError) {
            toast.error(courseCompleteError?.data?.message || "Failed to complete course something went wrong")
        }
    }, [courseCompleteData, isSuccess, courseCompleteError, courseInCompleteData, courseInCompleteIssuccess])

    useEffect(() => {

        if (courseInCompleteIssuccess) {
            toast.success(courseInCompleteData.message || "course marked as incompleted successfully")
            refetch()
        }
    ...
    const initialLecture = currentLecture || course.lectures && course.lectures[0]
                ...
                <h2 className='text-xl font-semibold'>Lectures</h2>
                {
                    course.lectures.map((lecture) => (
                        ...
                        >
                            ...
                            <div>
                                {handleLectureCompleted(lecture._id) && <span className='bg-green-200 text-green-800 rounded-md px-2 '>Completed</span>}
                            </div>
                        </div>
                    ))}
