...
import {
  ...
  useGetUserCourseRatingByIdQuery,
...
const RatingCard = ({ courseId, setShowCard }) => {
  ...
  const [CourseReview, { data, isLoading, isSuccess, error }] =
    useCourseReviewMutation();
  const { data: userCourseReview } = useGetUserCourseRatingByIdQuery(courseId);
  ...
  useEffect(() => {
    if (isSuccess) {
      toast.success(
        data?.message || "Thanks for your rating! your rating has been recorded"
      );
    }
    if (error) {
      toast.error(
        error.data?.message || "something is wrong ! Try again after some time"
      );
    }
    ...
    <Card className=" w-auto sm:w-[400px] shadow-lg  ">
      <CardHeader>
        ...
        <CardDescription className="text-yellow-500 dark:text-yellow-300">
          You can only rate or review this course once !
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Rating
      ...
      <CardFooter className="flex justify-between">
        <button
          ...
          className="px-3 py-1 bg-gray-300 rounded-lg hover:bg-gray-400 dark:bg-gray-600 dark:hover:bg-gray-800"
        >
          Cancel
        </button>
        <button
          onClick={handleSubmitReviewAndRating}
